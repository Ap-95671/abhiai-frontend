...
(async()=>{
 ...
 try{
  ...
  await panel.getByRole('button',{name:'Assistant settings and memory'}).click();await panel.getByRole('combobox',{name:'Assistant mode',exact:true}).click();await panel.getByRole('option',{name:'TUTOR',exact:true}).click();await page.waitForFunction(()=>!document.querySelector('fieldset')?.disabled);assert.equal(f.state.preferences.mode,'TUTOR');
