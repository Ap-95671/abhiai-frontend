...
export function AssistantComposer({ disabled, onSend, maxLength=10000 }: { disabled: boolean; maxLength?:number; onSend(content: string): Promise<boolean> }) {
  ...
  return <form className={styles.composer} onSubmit={event => { event.preventDefault(); void submit(); }}>
    ...
    <textarea id="assistant-message" rows={2} maxLength={maxLength} placeholder="Ask AbhiAI…" value={draft}
